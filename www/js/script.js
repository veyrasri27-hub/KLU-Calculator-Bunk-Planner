document.addEventListener('DOMContentLoaded', () => {
  ...
  let globalState = JSON.parse(localStorage.getItem('attendance-planner-state') || '{}');
  ...
  componentsList[type].forEach(comp => {
    ...
    html += `
      <div class="component-card">
        ...
        <label>Conducted: <input type="number" class="conducted" id="cond-${safeCompName}" name="cond-${safeCompName}" data-comp="${comp}" min="0" autocomplete="${getSavedInputValue(comp, 'cond') ? 'do-not-cond' : 'off'}" value="${getSavedInputValue(comp, 'cond')}"></label>
        <label>Attended: <input type="number" class="attended" id="attn-${safeCompName}" name="attn-${safeCompName}" data-comp="${comp}" min="0" autocomplete="${getSavedInputValue(comp, 'attn') ? 'do-not-attn' : 'off'}" value="${getSavedInputValue(comp, 'attn')}"></label>
      </div>`;
  });
...
function renderRemainingInputs() {
  ...
  if (method === 'direct') {
    componentsList[type].forEach(comp => {
      const safeCompName = comp.toLowerCase().replace(/\s+/g, '-');
      html.push(`<label>${comp} future classes: <input type="number" class="future" id="fut-${safeCompName}" name="fut-${safeCompName}" data-comp="${comp}" min="0" autocomplete="${getSavedInputValue(comp, 'fut') ? 'do-not-fut' : 'off'}" value="${getSavedInputValue(comp, 'fut')}"></label>`);
    });
  } else {
    const unitLabel = method === 'weeks' ? 'Weeks remaining' : 'Months remaining';
    html.push(`<label>${unitLabel}: <input type="number" id="remain-duration" name="remain-duration" min="0" autocomplete="${getSavedInputValue('none', 'misc', 'remain-duration') ? 'do-not-duration' : 'off'}" value="${getSavedInputValue('none', 'misc', 'remain-duration')}"></label>`);
    componentsList[type].forEach(comp => {
      const safeCompName = comp.toLowerCase().replace(/\s+/g, '-');
      html.push(`<label>${comp} per ${method === 'weeks' ? 'week' : 'month'}: <input type="number" class="per-unit" id="unit-${safeCompName}" name="unit-${safeCompName}" data-comp="${comp}" min="0" autocomplete="${getSavedInputValue(comp, 'unit') ? 'do-not-unit' : 'off'}" value="${getSavedInputValue(comp, 'unit')}"></label>`);
    });
  }
